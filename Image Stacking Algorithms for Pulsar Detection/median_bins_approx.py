...
def median_bins_fits(filenames, B):
  ...
  # Loops over all FITS files
  for filename in filenames:
      hdulist = fits.open(filename)
      data = hdulist[0].data

      # Loops over every point in the 2D array
...
def median_approx_fits(filenames, B):
  ...
  bin_width = 2*std / B
  # Calculates the approximated median for each array element
  median = np.zeros(dim)   
  for i in range(dim[0]):
    for j in range(dim[1]):    
      count = left_bin[i, j]
      for b, bincount in enumerate(bins[i, j]):
        count += bincount
        if count >= mid:
          # Stops when the cumulative count exceeds the midpoint
          break
